...
if [ -z ${JDK_HOME} ]; then JDK_HOME="/usr/lib/jvm/java-8-oracle"; fi
...
g++ -shared -fPIC  -Wall -o dist/libOpenKinect.so ${JNI_SRC_DIR}/org_openkinect_Context.cpp ${JNI_SRC_DIR}/org_openkinect_Device.cpp ${LIBFREENET_INCLUDE} ${LIBFREENET_LIBRARY} ${LIBUSB_INCLUDE} ${LIBUSB_LIBRARY} -I${JDK_HOME}/include/ -I${JDK_HOME}/include/linux/ -L${JDK_HOME}/lib/ 
